...
####################################
#
#utm log collector for proxy issues 
#
####################################
...
utm_outcoming_interface=$(ip -o route get $destination_ip | perl -nle 'if ( /dev\s+(\S+)/ ) {print $1}')
...
#enable debug for proxy module
cc -noquote set '"http"' '"debug"' '["all"]'    
/var/mdw/scripts/httpproxy restart              

sleep 20

#### log collection commands ####
tcpdump -envvi $utm_incoming_interface host $source_ip and port $dst_port -s0 -w /tmp/$case_number/capture_utm_incoming_interface_$utm_incoming_interface_$(date +"%Y_%m_%d_%I_%M_%p").log.pcap -b &
program_1_pid=$! 

tcpdump -envvi $utm_outgoing_interface host $source_ip and port $dst_port -s0 -w /tmp/$case_number/capture_utm_incoming_interface_$utm_outgoing_interface_$(date +"%Y_%m_%d_%I_%M_%p").log.pcap -b &
program_2_pid=$! 
...
tail -f /var/log/httpd.log > /tmp/$case_number/httpd_$(date +"%Y_%m_%d_%I_%M_%p").log &
program_5_pid=$!

tail -f /var/chroot-http/var/tmp/httpout > /tmp/$case_number/http_$(date +"%Y_%m_%d_%I_%M_%p").log &
program_6_pid=$!
...
echo tail -f /var/log/httpd.log >> /tmp/$case_number/report_$case_number.txt 
echo tail -f /var/log/http.log >> /tmp/$case_number/report_$case_number.txt 
