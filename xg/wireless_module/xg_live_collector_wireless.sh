...
#### compress everything ####
tar -czvf /tmp/${case_number}_compressed.tar.gz.tar.gz /tmp/${case_number}/ > /dev/null 2>&1        #compress the folder to export

#### sends logs to sophos ftp server ####
echo "RUN THE FOLLOWING COMMANDS TO SEND THE LOGS TO SOPHOS FTP SERVER"
echo "      curl --ftp-ssl ftp://ftp.sophos.com:990 -u ${ftp_username}:${ftp_password} -v -T {/tmp/${case_number}_compressed.tar.gz.tar.gz}"
echo -en '\n'
echo -en '\n'